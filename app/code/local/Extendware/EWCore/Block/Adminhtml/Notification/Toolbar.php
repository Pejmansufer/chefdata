		...
		if ($collection->count() > 0) {
			...
			if ($item) {			
				...
				$messages[] = array(
					...
					'right' => $this->__('You have %s unread message(s). <a href="%s">Go to messages inbox</a>', $stats, $this->getUrl('extendware_ewcore/adminhtml_message/index'))
	    		);
			}
		}
		...
		if ($this->mHelper('config')->isDisabledExtensionsMessageEnabled() === true) {
	    	...
	    	if ($numDisabledModules >= $this->mHelper('config')->getDisabledExtensionsMinNum()) {
	    		$messages[] = array(
			    	...
			    	'info_url' => $this->getUrl('extendware_ewcore/adminhtml_module/index'),
