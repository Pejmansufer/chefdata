...
//array_keys($arr[array_keys($arr)[0]])[0]
        ...
        if ($show) {			
			...
			$shippingPerProductPrice = $this->getShippingPerProductPrice($request);
	...
	public function getShippingPerProductPrice(Mage_Shipping_Model_Rate_Request $request){
		...
		if ($request->getAllItems()) {
			...
			return $shippingPerProductPrice;
		}	
    ...
    protected function _getLiftgateRate($rate)
    {
		...
		$shippingPerProductPrice = $this->getShippingPerProductPrice($rate);
