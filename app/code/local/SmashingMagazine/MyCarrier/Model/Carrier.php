    ...
    {		
        ...
        if ($request->getAllItems()) {
            foreach ($request->getAllItems() as $item) {
				...
				if($productObj->getWidth() > 80 && $productObj->getAttributeText('manufacturer') == 'Marc Refrigeration' && $liftgateExist == false) $this->LiftgateAvailable = false;
				else {
					$liftgateExist = true;
					$this->LiftgateAvailable = true;
				}
				...
				else if(($productObj->getWeight() >= 50 && $productObj->getData('shipping_method') != 'ups') || $productObj->getData('shipping_method') == 'freight' || $productObj->getAttributeText('freeshipping') == 'Free Shipping')
                {
        ...
        <soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:wwex="http://www.wwexship.com">
           <soapenv:Header>
              <wwex:AuthenticationToken>
                 ...
                 <wwex:password>Worldwide1</wwex:password>
    ...
    protected function _getFreightRate(Mage_Shipping_Model_Rate_Request $request)
    {
        ...
        if ($request->getAllItems()) {
            foreach ($request->getAllItems() as $item) {
				...
				$shipCost = $productObj->getShipCost();
                ...
                else if (($productObj->getWeight() < 50 && $productObj->getData('shipping_method') != 'freight' && $productObj->getAttributeText('freeshipping') != 'Free Shipping') || $productObj->getData('shipping_method') == 'ups') {
					$this->UpsAvailable = true;
                    continue;
                }
...
//array_keys($arr[array_keys($arr)[0]])[0]
        ...
        foreach ($quote as $q) {
            if ($q->freightShipmentQuoteResult[2] != null)
                $total += $q->freightShipmentQuoteResult[2]->totalPrice * 1.2;
			else if ($q->freightShipmentQuoteResult[1] != null)
                $total += $q->freightShipmentQuoteResult[1]->totalPrice * 1.2;
        ...
        if ($show) {			
			...
			$shippingPerProductData = $this->getShippingPerProductPrice($request);
			$shippingPerProductPrice = $shippingPerProductData['price'];
			if($shippingPerProductData['flag'] == 1) {
				$rate->setPrice($shippingPerProductPrice);
			}else {
				$rate->setPrice($total + $this->UpsRate + $shippingPerProductPrice);
			}
	...
	public function getShippingPerProductPrice(Mage_Shipping_Model_Rate_Request $request){
		
		if ($request->getAllItems()) {
			$shipCost = 0;
			$shippingPerProductPrice = 0;
			$flag = 0;
			$check = 0;
			$data = array();
            foreach ($request->getAllItems() as $item) {
				...
				if($shipCost == null || $shipCost == 0 ){					
					...
					$flag = 1;
					$shippingPerProductPrice += $item->getQty() * $shipCost;
				}
			}
			
			if($check == 1 && count($request->getAllItems()) > 1) {
				$flag = 0;
			}
			$data['price'] = $shippingPerProductPrice;
			$data['flag'] = $flag; 			
			return $data;
		}	
    ...
    protected function _getLiftgateRate($rate)
    {
		...
		$shippingPerProductData = $this->getShippingPerProductPrice($rate);
		$shippingPerProductPrice = $shippingPerProductData['price'];
		...
		if($shippingPerProductData['flag'] == 1) {
			$rate->setPrice($shippingPerProductPrice+50);
		}else {
			$rate->setPrice($this->FreightRate + 50 * $this->ManufacturerCount + $shippingPerProductPrice);
		}
    ...
    protected function _getLiftgateResidentialRate($rate)
    {
		...
		if($shippingPerProductData['flag'] == 1) {
			$rate->setPrice($shippingPerProductPrice+110);
		}else {
			$rate->setPrice($this->FreightRate + 110 * $this->ManufacturerCount + $shippingPerProductPrice);
		}
