 ...
 */
class MS_ProductDetailShipping_Model_Estimate
{
    /**
     * Customer object,
    ...
    public function estimate()
    {
        ...
        if (!($product instanceof Mage_Catalog_Model_Product) || !$product->getId()) {
            Mage::throwException(
                Mage::helper('ms_productdetailshipping')->__('Please specify a valid product')
            );
        }

        if (!isset($addressInfo['country_id'])) {
            Mage::throwException(
                Mage::helper('ms_productdetailshipping')->__('Please specify a country')
            );
        }
        ...
        if (isset($addressInfo['postcode'])) {
            $shippingAddress->setPostcode(substr($addressInfo['postcode'], 0, 5));
        }
        ...
        if (isset($addressInfo['region_code'])) {
            $shippingAddress->setRegionCode($addressInfo['region_code']);
        } else {
            $state = Mage::getModel('ms_productdetailshipping/zipcodes')
                ->getStateByCountry(substr($addressInfo['postcode'], 0, 5), $addressInfo['country_id']);
            if ($state) {
			    $shippingAddress->setRegionCode($state);
                $addToCartInfo['region_code'] = $state;
            }
        }
    ...
    public function getQuote()
    {
        if ($this->_quote === null) {
            ...
            if (!empty($addressInfo['cart'])) {
                $quote = Mage::getSingleton('checkout/session')->getQuote(); //hide by nishit
            } else {
                $quote = Mage::getModel('sales/quote'); //hide by nishit
            }
            $this->_quote = $quote;
        }
