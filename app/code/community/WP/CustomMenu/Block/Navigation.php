...
class WP_CustomMenu_Block_Navigation extends Mage_Catalog_Block_Navigation
{
    ...
    public function drawMenuItem($children, $level = 1)
    {
        ...
        foreach ($children as $child) {
            ...
            if($tmp >= 5 && isset($child->getParent()->_data['url_path']))
            {
                $tempurl = Mage::getBaseUrl() . $child->getParent()->_data['url_path'];
                $cur_category2 = Mage::getModel('catalog/category')->load($child->getId());
            ...
            if (is_object($child) && $child->getIsActive()) {
                ...
                // --- format category name ---
                $name = $this->escapeHtml($child->getName());
                $image = Mage::getModel('catalog/category')->load($child->getId())->getImageUrl();
                if (Mage::getStoreConfig('custom_menu/general/non_breaking_space')) $name = str_replace(' ', '&nbsp;', $name);

                    $html.= '<div class="col-sm-2"><a href="' . $this->getCategoryUrl($child) . '"><div class="dd-menu-inner"><img class="img-responsive" src="' . $image . '" alt="' . $name . '"></div><div class="row">' . $name . '</div></a></div>';
                    $tmp++;
        ...
        $html.='<div class="col-sm-2 text-center"><a href="' . $tempurl . '"><em class="fa fa-plus"></em><div class="row">See More</div></a></div>';
        return $html;
    }
    ...
    {
        $html = '';
        $chunks = $this->_explodeByColumns($children, 1);
        $html.= '<div class="row-fluid">';
        $html.= $this->drawMenuItem($chunks[0], 1);
        $html.= '</div>';
        return $html;
    }
