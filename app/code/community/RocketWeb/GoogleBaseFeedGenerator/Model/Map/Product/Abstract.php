...
class RocketWeb_GoogleBaseFeedGenerator_Model_Map_Product_Abstract extends Varien_Object
{
    ...
    public function mapColumn($column)
    {
        ...
        if ($this->getIsApparel() && $this->hasParentMap()) {
    ...
    {
        switch ($type) {
            case RocketWeb_GoogleBaseFeedGenerator_Model_Source_Assocprods::FROM_PARENT:
                $value = $this->getParentMap() ? $this->getParentMap()->mapColumn($params['map']['column']) : '';
                break;
            case RocketWeb_GoogleBaseFeedGenerator_Model_Source_Assocprods::FROM_ASSOCIATED:
                $value = $this->getCellValue($params);
                break;
            case RocketWeb_GoogleBaseFeedGenerator_Model_Source_Assocprods::FROM_ASSOCIATED_PARENT:
                $value = $this->getCellValue($params);
                // Run replace empty rules if no value so far
                if ($value == "") {
                ...
                break;
            case RocketWeb_GoogleBaseFeedGenerator_Model_Source_Assocprods::FROM_PARENT_ASSOCIATED:
                $value = $this->getParentMap() ? $this->getParentMap()->mapColumn($params['map']['column']) : '';
                if ($value == '') {
                    $value = $this->getCellValue($params);
                }
    ...
    protected function mapDirectiveUrl($params = array())
    {
        // @var $product Mage_Catalog_Model_Product
        $product = $this->getProduct();
        $add_to_url = $params['map']['param'];

        $url = $product->getProductUrl();
//        $urlInstance = $product->getUrlModel()->getUrlInstance();
//        $routeParams['id'] = $product->getId();
//        $routeParams['s'] = $product->getUrlKey();
//        $url = $urlInstance->setData('route_front_name', 'standard')
//                           ->getUrl('catalog/product/view', $routeParams);
    ...
    protected function mapDirectiveSalePrice($params = array())
    {
        ...
        if (!$this->getTools()->isModuleEnabled('OrganicInternet_SimpleConfigurableProducts') &&
            ($this->hasParentMap() && $this->getParentMap()->getProduct()->getTypeId() == Mage_Catalog_Model_Product_Type::TYPE_CONFIGURABLE)
        ) {
        ...
        $this->_cache_sale_price_excluding_tax = $helper->getPrice($product, $price, false, false, false, null, $this->getStoreId(), $priceIncludesTax);
        $this->_cache_sale_price_including_tax = $helper->getPrice($product, $price, true, false, false, null, $this->getStoreId(), $priceIncludesTax);

        $price = $includingTax ? $this->_cache_sale_price_including_tax : $this->_cache_sale_price_excluding_tax;
        if ($price <= 0) {
            $this->log(sprintf("product id %d product sku %s, skipped - product has sale_price '%s'.", $product->getId(), $product->getSku(), $price));
        }
    ...
    protected function mapDirectiveShippingWeight($params = array())
    {
		...
		// Cafer Edit
		if(((float)str_replace(',','',$weight))>2000) $weight = '1,900.00';
		// End Cafer Edit
        $weight .= $weight ? ' ' . $unit : '';
    ...
    protected function cleanField($field, $params = null)
    {
        ...
        if (extension_loaded('mbstring')) {
            $field = preg_replace_callback("/(&#?[a-z0-9]{2,8};)/i", array($this, '__htmlentities_to_utf8'), $field);
        }
    ...
    private function __htmlentities_to_utf8($m) { return mb_convert_encoding($m[1], "UTF-8", "HTML-ENTITIES"); }
     ...
     */
    public function checkSkipSubmission($nodups = false)
    {
        if ($this->getProduct()->getData('rw_google_base_skip_submi') == 1) {
            $this->skip = true;
            if ($this->getConfigVar('log_skip')) {
                $this->log(sprintf("product id %d sku %s, configurable associated, skipped - product has 'Skip from Being Submitted' = 'Yes'.", $this->getProduct()->getId(), $this->getProduct()->getSku()));
            }
        }

        if (!$nodups && $this->isDuplicate()) {
            $this->skip = true;
        }
    ...
    public function setSkip($skip_message)
    {
        if ($this->getConfigVar('auto_skip')) {
            $this->skip = true;
            $this->getGenerator()->updateCountSkip();
