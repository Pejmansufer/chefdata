...
class RocketWeb_GoogleBaseFeedGenerator_Model_Tools extends Varien_Object
{
     ...
     * TODO: check if parent disabled at store level
     *
     * @param  $type_id
     * @param  $sku
     * @param  null $store_id
    ...
    public function isChildOfConfigurable($type_id, $sku, $store_id = null)
    {
        ...
        $sql = "SELECT
				...
				INNER JOIN `" . $this->getRes()->getTableName('catalog/product') . "` AS `cpe_parent`
					ON ( `cpsl`.`parent_id`=`cpe_parent`.`entity_id` AND `cpe_parent`.`entity_id`)
				INNER JOIN `" . $this->getRes()->getTableName('catalog/product') . "_int` AS `cpe_parent_status`
        ...
        return $this->getConnRead()->fetchAll($sql);
    }
    ...
    public function setCacheAssociatedPricesByProduct($map, $assoc)
    {
        ...
        if ((!$has_option_price && count($configurable_attributes)) || $this->isModuleEnabled('OrganicInternet_SimpleConfigurableProducts')) {
    ...
    public function lockDuplicates($row, $is_assoc_configurable, $is_assoc_grouped)
    {
        ...
        if (!empty($is_assoc_configurable)
            ...
            if ($parents) {
                if ($generator->getConfigVar('log_skip')) {
                    $prods_msg = ': ';
