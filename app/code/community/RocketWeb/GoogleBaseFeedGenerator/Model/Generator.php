...
class RocketWeb_GoogleBaseFeedGenerator_Model_Generator extends Varien_Object
{
     ...
     * @param bool $is_assoc_configurable
     * @return array
     */
    public function generateProductMap($product) {
        if (!$this->_base_map) {
            $this->_base_map = $this->getProductMapModel('abstract')
                ->setColumnsMap($this->getColumnsMap())
                ->setEmptyColumnsReplaceMap($this->getEmptyColumnsReplaceMap())
                ->initialize();
        }

        $category_ids = $this->getTools()->getCategoriesById($product->getId());
        $is_apparel = $this->_base_map->isApparelBySql($product->getId(), null, $category_ids);

        $productMap = $this->getProductMapModel($product->getTypeId(), array(
            'is_assoc_configurable' => false,
            'is_assoc_grouped' => false,
            'is_apparel'  => (@$is_apparel == 1 ? true : false),
        ));

        //Skip product if not provided
        if($productMap == false) {
            return array();
        }
    ...
    public function getProductMapModel($type_id, $args = array())
    {
        ...
        case 'simple':
            ...
            if ($is_assoc_configurable && $this->isProductTypeEnabled(Mage_Catalog_Model_Product_Type::TYPE_CONFIGURABLE) && !$is_apparel) {
        ...
        if (is_null($productMap) && class_exists('RocketWeb_GoogleBaseFeedGenerator_Model_Map_Product_' . ucfirst($type_id)) ) {
    ...
    public function processProductCallback($args)
    {
        ...
        if (($is_assoc_configurable || $is_assoc_grouped)) {
            if ($this->getTools()->lockDuplicates($row, $is_assoc_configurable, $is_assoc_grouped) && !$this->getTestMode()) {
                return;
            }
        }

        $category_ids = $this->getTools()->getCategoriesById($row['entity_id']);
        ...
        if ($is_apparel && !$this->getConfigVar('is_turned_on', 'apparel')) {
            if ($this->getConfigVar('log_skip', 'file')) {
        ...
        $productMap = $this->getProductMapModel(
            $row['type_id'], array(
    ...
    protected function writeFeed($fields, $add_new_line = true)
    {
        ...
        foreach ($fields as $key => $value) {
            $fields[$key] = filter_var($value, FILTER_UNSAFE_RAW, FILTER_FLAG_STRIP_LOW|FILTER_FLAG_STRIP_HIGH);
        }
